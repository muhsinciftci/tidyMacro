...
| `shock_size`   | ✓   | ✓     | —        | —      | `"unit"` (default) or `"sd"` impulse units |
...
Every engine accepts a **vector** `conf`, e.g. `conf = c(68, 95)`, and rebuilds every band from the same raw SE in R. There is no re-estimation cost and no multiple-testing story — bands are just alternative visualizations of the same variance. `fPlotLP()` draws one ribbon per level automatically.

### 2.2 · Impulse units (`shock_size`)

`fLP()` and `fLPIV()` report the response to a **one-unit** shock by default (`shock_size = "unit"`). Setting `shock_size = "sd"` reports the response to a **one-standard-deviation** shock instead: `fLP()` z-scores the shock regressor, `fLPIV()` scales the 2SLS coefficient and its standard error by `sd(D)`. In both cases the standard deviation is taken over the **estimation sample** — i.e. after the leading rows consumed by generated lags — not over the raw input column. These correspond to `LPopt.impact = 1` and `LPopt.impact = 0` in Cesa-Bianchi's `LPmodel.m`.

### 2.3 · Threading and console noise
...
A tibble of class `c("fLPDID", ...)` with columns `event_time, estimate, se, conf_low, conf_high, nobs, nclust, ndrop`, and an `attr("lpdid_spec")` list capturing every argument that shaped the fit. `ndrop` counts control columns omitted for collinearity at that horizon (as `reghdfe` does); horizons that could not be estimated raise a warning naming the event times rather than returning a silent `NaN`. The plot function is `fPlotLPDID()`, which accepts one result or several named results (`fPlotLPDID(\`OLS\` = m1, \`Reweighted\` = m2)`).
...
The base $y_{t-1}$ for the **first** estimable date lies one period before the estimation sample. When generated lags trim the leading rows, that outcome is still observed, so `fLP()` and `fLPIV()` keep it as the base rather than discarding it — the estimation sample is not shortened by the long difference. This matches `endo_lag1 = ENDO(lag_trim:end-1)` in `LPmodel.m`. Time-series LP also requires a contiguous sample: rows with `NA` cannot simply be deleted, since the engines treat retained rows as consecutive periods, so an **interior** gap is rejected with an error (leading and trailing `NA`s from `l()` / `f()` are fine).
...
restricted to a clean-control sample. Pre-treatment horizons replace $y_{i, t+h}$ by $y_{i, t-j}$ with $\beta_{-1} = 0$ as the normalization. Standard errors are CR1 cluster-robust with the `reghdfe` correction $c = \tfrac{G}{G-1} \cdot \tfrac{N-1}{N-K}$ where $K$ counts absorbed time fixed effects. Singleton absorbed-FE observations are dropped before $N$ and $K$ are formed, again as `reghdfe` does. Confidence bands use a cluster-$t$ critical value with $G-1$ degrees of freedom, not a normal quantile.
