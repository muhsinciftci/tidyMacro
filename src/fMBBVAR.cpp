...
// ---------------------------------------------------------------------------
// Original fMBBVAR_cpp — Nullable overload
// ---------------------------------------------------------------------------
// Internal C++ function (called from other C++ code)
MBBVARResult fMBBVAR_cpp(const arma::mat& eps, 
                          int lags, 
                          int BlockSize, 
...
//' result <- fMBBVAR(eps, lags = 2, BlockSize = 0)
//' eps_boot <- result$eps_boot
//' 
//' # Bootstrap residuals and instruments
//' M <- matrix(rnorm(100), 100, 1)
//' result <- fMBBVAR(eps, lags = 2, BlockSize = 0, M)
//' M_boot <- result$M_boot
//' }
//' 
//' @export
// [[Rcpp::export]]
Rcpp::List fMBBVAR(const arma::mat& eps, 
                    int lags, 
                    int BlockSize, 
                    Rcpp::Nullable<arma::mat> M = R_NilValue) {
  // Call the C++ function
  MBBVARResult result = fMBBVAR_cpp(eps, lags, BlockSize, M);
